...
def index():
    ...
    if request.method == 'POST':
        ...
        if action:
            prompt = ("Use the reference ID TXLnYeq6rsLqVTTa, uvpARKZcAUCKOBn7 and P3PUOoDqFsKHJXAA. Medium: two-dimensional, shield-like shaped, very detailed sports vector emblem only. Subject: A {animal} mascot for {sport}, depicting intricate detail in the sport's attire and equipment.  The {animal} is depicted {action}. Emotion: {personality} Lighting: Flat, Vivid and bold,  with a primary focus on {color}. White background. Scene: Surrounded by Pure white background, highlighted by a prominent, heavy black border. Style: Sharp, clean lines, bold use of {color}, modern design --ar 1:1 View: front view")
        else:
