...
def index():
    ...
    if request.method == 'POST':
        ...
        if action:
            prompt = ("Design a new emblem based on the image with the reference image ID [rdLgknl5CNR6R0m7]. The emblem must be very detailed, two-dimensional, non-symmetrical, featuring an {animal} mascot for {sport}. The only object in the render should be this single emblem. The emblem should be outlined in a bold, thick line styling to stand out on the white background. The {animal} must have a {personality} facial expression and express that its {action} pose. Use a striking {color} to accentuate the key elements of the {animal}'s sportswear and equipment for the sport that the {animal} represents being {sport}. {color} is the prominant color of the emblem, with thick striking black outlines. The design must be vibrant and expressive, capturing the movement and spirit of {sport}. The {animal}'s features should be realistic and real.")
        else:
...
def vectorize_image():
    ...
    # Step 1: Get the original image path in 'static/raw'
